...
int main(int argc, char ** argv) {
    ...
    console::log("  /search <query>     web search (DuckDuckGo)\n");
    console::log("  /date               current date\n");
    console::log("  /time [timestamp]   current time or Unix timestamp\n");
    console::log("  /agent <task>       execute autonomous task\n");
    console::log("  /online images      generate image with Hugging Face\n");
    console::log("  /online audios      generate audio with Hugging Face\n");
    console::log("  /online code        generate code with Hugging Face\n");
    console::log("  /online videos      generate video frame with Hugging Face\n");
    ...
    while (true) {
        ...
        if (string_starts_with(buffer, "/exit")) {
            ...
            is_command = true;
        } else if (string_starts_with(buffer, "/date")) {
            std::string result = llama_tools::cmd_date();
            console::log("%s\n", result.c_str());
            is_command = true;
        } else if (string_starts_with(buffer, "/time")) {
            std::string result = llama_tools::cmd_time(0);
            console::log("%s\n", result.c_str());
            is_command = true;
        } else if (string_starts_with(buffer, "/search ")) {
            std::string query = string_strip(buffer.substr(8));
            if (query.empty()) {
            ...
            std::string search_result = llama_tools::cmd_search(query);
            console::log("%s\n", search_result.c_str());
            is_command = true;
        } else if (string_starts_with(buffer, "/agent ")) {
            std::string task = string_strip(buffer.substr(7));
            if (task.empty()) {
                console::error("Usage: /agent <task>\n");
                continue;
            }
            if (task.front() == '"' && task.back() == '"') {
                task = task.substr(1, task.length() - 2);
            }
            std::string agent_result = llama_tools::cmd_agent(task);
            console::log("%s\n", agent_result.c_str());
            is_command = true;
        ...
        } else if (string_starts_with(buffer, "/online ")) {
            std::string subcmd = string_strip(buffer.substr(8));
            if (subcmd.empty()) {
                console::error("Usage: /online images|audios|code|videos <prompt>\n");
                continue;
            }
            
            // Извлекаем тип и промпт
            ...
            std::string type = subcmd.substr(0, space_pos);
            std::string prompt = string_strip(subcmd.substr(space_pos + 1));
            ...
            std::string result = llama_tools::process_online_command(type, prompt);
            console::log("%s\n", result.c_str());
            is_command = true;
        ...
        // Если это была команда без необходимости генерации - пропускаем
        if (is_command) {
            continue;
        }

        // generate response only for non-command inputs
        if (add_user_msg) {
            ctx_cli.messages.push_back({
                {"role",    "user"},
                {"content", cur_msg}
